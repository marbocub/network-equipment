...
class ResponseParser
{
    ...
    public function parse($command, $responseText, $lineEnding = "\n")
    {
        /*
         * Cisco: show interface status
         */
        if (preg_match("/(s|sh(o|ow)?)\s+int(e(r(f(a(c(e)?)?)?)?)?)?\s+status?.*/", $command)) {
            return $this->parseShowInterfaceStatus($responseText, $lineEnding);
        }
        ...
        if (preg_match("/(s|sh(o|ow)?)\s+int(e(r(f(a(c(e)?)?)?)?)?)?\s+co(u(n(t(e(r(s)?)?)?)?)?)?.*/", $command)) {
            return $this->parseShowInterfaceCounters($responseText, $lineEnding);
        }
        ...
        if (preg_match("/(s|sh(o|ow)?)\s+mac\s+ad(d(r(e(s(s(-(t(a(b(l(e)?)?)?)?)?)?)?)?)?)?)?.*/", $command)) {
            return $this->parseShowMacAddressTable($responseText, $lineEnding);
        }
        ...
        if (preg_match("/(s|sh(o|ow)?)\s+ll(d(p)?)?\s+n(e(i(g(h(b(o(r(s)?)?)?)?)?)?)?)?.*/", $command)) {
            return $this->parseShowLldpNeighbors($responseText, $lineEnding);
        }
        ...
        if (preg_match("/(s|sh(o|ow)?)\s+ip\s+in(t(e(r(f(a(c(e)?)?)?)?)?)?)?\s+br(r(i(e(f)?)?)?)?.*/", $command)) {
            return $this->parseShowIpInterfaceBrief($responseText, $lineEnding);
        }
        ...
        if (preg_match("/(s|sh(o|ow)?)\s+arp?.*/", $command)) {
            return $this->parseShowArp($responseText, $lineEnding);
        }
